function [ y ] = fla_trmv( uplo, trans, diag, A, x )

% fla_trmv( uplo, trans, diag, A, y )
%    fla_trmv( 'Lower triangular', 'No transpose', 'Nonunit diag', A, y )
%        computes y := tril( A ) * x
%    fla_trmv( 'Upper triangular', 'Transpose', 'Nonunit diag', A, y )
%        computes y = triu( A )' * x 
%    fla_trmv( 'Lower triangular', 'No transpose', 'Unit diag', A, y )
...
% size of A.
assert( m_A == n_A && m_A == m_x, 'trmv: A and/or x do not have same size' );
